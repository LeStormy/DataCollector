	...
	async getWeatherInCityByDate(city, country, date) {
		...
		if (res.matches_count > 0) {
			res.items.forEach(function(e) {
				tmp = [e.start_datetime.datetime.substring(e.start_datetime.datetime.indexOf('T') + 1, e.start_datetime.datetime.indexOf('+')),
	...
	async getNextRainyDaysInCity(city, country) {
  	...
  	async getSoccerGames(country) {
		...
		if (res.matches_count > 0) {
			res.items.forEach(function(e) {
				var tmp = [e.start_datetime.datetime.substring(0, e.start_datetime.datetime.indexOf('T')),
					e.start_datetime.datetime.substring(e.start_datetime.datetime.indexOf('T') + 1, e.start_datetime.datetime.indexOf('+')),
		...
		games.forEach(function(e) {
			data += "{\"date\": \"" + e[0] + "\", \"time\": \"" + e[1] + "\", \"home_team\": \"" + e[2] + "\", \"away_team\": \"" + e[3] + "\"}, ";
		});
		data = data.substring(0, data.length - 2);
		data += "]";
